function status = stExamplesShow(theFunction,varargin)
% Open file, read it, parse and print the examples to the command window
%
% Syntax:
%     status = stExamplesShow(theFunction, varargin)
%
% Description:
...
%
% Inputs:
%    theFunction - A string that is either
%      * a function name on the user's path. In general,
%       which(theFunction) should return the full path to the function, OR
%      * A class method, such as 'scitran.exist'.  In this case, the
%      examples from the method in @scitran/exist are shown in the command
%      window. 
%
% Outputs:
...
%    'verbose' -      Boolean. Prints alerts to command line. Default false
%
...
%{
  % Should execute both examples successfully
    stExamplesShow('stExamplesShow')
    stExamplesShow('@scitran/containerGet');
    stExamplesShow('scitran.exist');
%}
...
for bb = 1:length(startIndices)
    cnt = cnt+1;
    fprintf('Example %d \n----------\n',cnt);
    % Find the end of line for the initial block comment part.  Sometimes
    % people put in extra spaces, so we need to actually find the EOL.

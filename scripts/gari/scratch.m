...
%%
cc            = st.search('collection','collection label exact',collectionName,'fw',true);
...
if isempty(collectionID)
    ...
    thisCollection        = cc{1}; % st.fw.getCollection(collectionID);
    sessions  = thisCollection.sessions(); % st.fw.getCollectionSessions(idGet(thisCollection));
    fprintf('There are %i sessions in the collection %s (server %s).\n', length(sessions), collectionName, st.instance)
    if showListSession
        for ns=1:length(sessions)
            thisSession = st.fw.getSession(idGet(sessions{ns}));
            % Get the project the session belongs to
            thisProject = st.fw.get(sessions{ns}.project());

...
[files,srchCmd] = st.search('file',...
    'acquisition id',idGet(acquisitions{1},'data type','acquisition'),...
    'summary',true);
...
analyses = st.search('analysis',...
    'session id',idGet(sessions{3},'data type','session'),...
    'summary',true);
...
for ii=1:length(sessions)
    analyses{ii} = st.search('analysis',...
        'session id',idGet(sessions{ii},'data type','session'),...
        'summary',true);
end

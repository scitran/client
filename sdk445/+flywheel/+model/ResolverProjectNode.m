...
%    struct - Convert the object to a struct
    ...
    properties (Constant)
        propertyMap = containers.Map({ 'public', 'label', 'info', 'description', 'group', 'id', 'parents', 'infoExists', 'created', 'modified', 'template', 'permissions', 'files', 'notes', 'tags', 'analyses' }, ...
            { 'public', 'label', 'info', 'description', 'group', '_id', 'parents', 'info_exists', 'created', 'modified', 'template', 'permissions', 'files', 'notes', 'tags', 'analyses' });
    end
    ...
    methods
        function obj = ResolverProjectNode(varargin)
            ...
            if length(varargin)
                ...
                if ~isempty(p.Results.template)
                    obj.props_('template') = p.Results.template;
                end
        ...
        function result = get.template(obj)
            if ismethod(obj, 'get_template')
                result = obj.get_template();
            else
                if isKey(obj.props_, 'template')
                    result = obj.props_('template');
                else
                    result = [];
                end
            end
        ...
        function result = toJson(obj)
            ...
            if isKey(obj.props_, 'template')
                result('template') = flywheel.ModelBase.serializeValue(obj.props_('template'), 'containers.Map');
            end
            if isKey(obj.props_, 'permissions')
        ...
        function result = struct(obj)
            ...
            if isKey(obj.props_, 'template')
                result.template = obj.props_('template');
            else
                result.template = [];
            end
            if isKey(obj.props_, 'permissions')
    ...
    methods(Access = protected)
        function prpgrp = getPropertyGroups(obj)
            if ~isscalar(obj)
                ...
                if isKey(obj.props_, 'template')
                    propList.template = obj.props_('template');
                else
                    propList.template = [];
                end
                if isKey(obj.props_, 'permissions')
    ...
    methods(Static)
        function obj = fromJson(json, context)
            ...
            if isfield(json, 'template')
                obj.props_('template') = flywheel.ModelBase.deserializeValue(json.template, 'containers.Map');
            end
